...
class TestGauged(TestCase):
    ...
    def test_keys(self):
        ...
        self.assertListEqual(gauged.keys(), [b'foobar', b'foobaz'])
        self.assertListEqual(gauged.keys(prefix='bar'), [])
        self.assertListEqual(gauged.keys(namespace=1), [b'bar'])
        self.assertListEqual(gauged.keys(namespace=1, prefix='bar'), [b'bar'])
        self.assertListEqual(gauged.keys(limit=1), [b'foobar'])
        self.assertListEqual(gauged.keys(limit=1, offset=1), [b'foobaz'])
    ...
    def test_parse_query(self):
        gauged = Gauged(self.driver)
        with gauged.writer as writer:
            result = writer.parse_query(b'foo=bar&baz=&foobar&%3Ckey%3E=value%3D%3')
            self.assertDictEqual(dict(result), {b'<key>': b'value=%3', b'foo': b'bar', b'baz': b''})

    def test_constant_accessibility(self):
        gauged = Gauged(self.driver)
        self.assertTrue(type(Gauged.HOUR) == int)
        self.assertTrue(type(gauged.HOUR) == int)
        self.assertTrue(type(Gauged.NOW) == int)
        self.assertTrue(type(gauged.NOW) == int)
    ...
    def test_gauge(self):
        ...
        self.assertCountEqual(gauged.keys(), [b'foobar', b'bar'])
    ...
    def test_gauge_long_key(self):
        gauged = Gauged(self.driver)
        long_key = ''
        for _ in range(self.driver.MAX_KEY + 1):
            long_key += 'a'
        with gauged.writer as writer:
    ...
    def test_context_defaults(self):
        ...
        self.assertListEqual(gauged.keys(), [b'bar', b'foo'])
        gauged = Gauged(self.driver, resolution=1000, block_size=10000, defaults={
            'limit': 1
        })
        self.assertListEqual(gauged.keys(), [b'bar'])
    ...
    def test_clear_key_after(self):
        ...
        self.assertEqual(gauged.value('foo', timestamp=40000, namespace=1), 4)
        self.assertEqual(gauged.value('bar', timestamp=40000, namespace=1), 4)
        self.assertEqual(gauged.value(b'bar', timestamp=40000, namespace=1), 4)
        # timestamp should be on a block boundary
        with gauged.writer as writer:
            with self.assertRaises(ValueError):
