...
class TestDriver(TestCase):
    ...
    def test_insert_blocks(self):
        ...
        self.assertEqual(self.driver.get_block(1, 2, 3)[0], b'bar')

        self.driver.replace_blocks([( 0,  1, 2, b'\xe4\x00\x12', 0x10 )])

        self.assertEqual(self.driver.get_block(0, 1, 2)[0], b'\xe4\x00\x12')

        self.driver.insert_or_append_blocks([( 0,  1, 2, b'\xe4\x00\x12', 0x10 )])

        self.assertEqual(self.driver.get_block(0, 1, 2)[0], b'\xe4\x00\x12\xe4\x00\x12')
    ...
    def test_remove_namespace(self):
        ...
        blocks = [( 0,  1, 2, b'foo', 0x10 ),
                  ( 1,  2, 3, b'bar', 0x10 )]
        self.driver.replace_blocks(blocks)
        self.assertEqual(self.driver.get_block(0, 1, 2)[0], b'foo')
        self.assertEqual(self.driver.get_block(1, 2, 3)[0], b'bar')
        self.driver.remove_namespace(1)
        self.assertEqual(self.driver.get_block(0, 1, 2)[0], b'foo')
    ...
    def test_namespace_statistics(self):
        ...
        self.assertEqual(min_block, 0)
        self.assertEqual(max_block, 1)
        self.assertCountEqual(self.driver.get_namespaces(), [0, 1])
